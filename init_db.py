...
def init_database():
    ...
    with app.app_context():
        ...
        kg_unit = Unit(name='Kilogram', abbreviation='kg', description='Weight unit in kilograms')
        piece_unit = Unit(name='Piece', abbreviation='piece', description='Count unit for individual items')
        
        db.session.add(kg_unit)
        db.session.add(piece_unit)
        db.session.commit()  # Get unit IDs
        
        # Create some sample ingredients
        coffee_beans = Ingredient(name='Coffee Beans', unit_id=kg_unit.id, current_stock=10.0, reorder_level=2.0)
        bread = Ingredient(name='Bread', unit_id=piece_unit.id, current_stock=50, reorder_level=10)
        flour = Ingredient(name='Flour', unit_id=kg_unit.id, current_stock=5.0, reorder_level=1.0)
