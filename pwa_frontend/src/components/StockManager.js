...
function IngredientsTab() {
  ...
  const [formData, setFormData] = useState({
    name: '', unit_id: '', cost_per_unit_usd: '', reorder_level: '', current_stock: ''
  });
  ...
  const fetchUnits = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/v1/units/`, { headers: getAuthHeader() });
  ...
  const resetForm = () => {
    setFormData({ name: '', unit_id: '', cost_per_unit_usd: '', reorder_level: '', current_stock: '' });
    setEditingId(null);
    setUsage([]);
  };
  ...
  const handleEdit = (ingredient) => {
    setFormData({
      ...ingredient,
      unit_id: ingredient.unit_id || ingredient.unit // Handle both old and new format
    });
