...
function RecipeManager({ menuItemId }) {
  ...
  const fetchRecipe = useCallback(async () => {
    ...
    try {
      const res = await axios.get(`${API_BASE_URL}/api/v1/menu/${menuItemId}/recipe`, { headers: getAuthHeader() });
      setRecipe(res.data);
    } catch (err) {
  ...
  const handleSaveRecipe = async () => {
    try {
      ...
      await axios.post(`${API_BASE_URL}/api/v1/menu/${menuItemId}/recipe`, payload, { headers: getAuthHeader() });
