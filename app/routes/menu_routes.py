...
def update_menu_item(item_id):
    ...
    try:
        ...
        try:
            ...
            current_app.logger.error(f"Validation error updating menu item {item_id}: {err.messages}")
            return jsonify({
                'message': 'Validation error',
                'errors': err.messages
            }), 400
        ...
        current_app.logger.info(f"Successfully updated menu item {item_id}. Updated fields: {', '.join(update_fields)}")
        return jsonify({
...
def get_menu_item_options(item_id):
    ...
    try:
        item = MenuItem.query.options(joinedload(MenuItem.options).joinedload(MenuItemOption.choices)).get_or_404(item_id)
        ...
        current_app.logger.error(f"Error fetching options for menu item {item_id}: {str(e)}")
...
def add_menu_item_option(item_id):
    ...
    try:
        ...
        db.session.rollback()
        current_app.logger.error(f"Error adding option for menu item {item_id}: {str(e)}")
