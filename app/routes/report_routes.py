...
from app.models import db, Order, OrderItem, Ingredient, StockAdjustment, OrderDiscount, OrderItemDiscount, MenuItem, Category, OrderStatus
...
def get_sales_summary():
    ...
    try:
        ...
        average_order_value_usd = total_sales_usd / total_orders if total_orders > 0 else 0
        ...
        ).select_from(MenuItem).join(OrderItem, MenuItem.id == OrderItem.menu_item_id).join(Order, OrderItem.order_id == Order.id).filter(
            Order.created_at >= start_date,
            Order.created_at < end_date,
            Order.status == OrderStatus.completed
        ...
        category_sales = db.session.query(
            ...
            func.sum(OrderItem.line_total_usd_at_order).label('total_revenue_usd')
        ).select_from(Category).join(MenuItem, Category.id == MenuItem.category_id).join(OrderItem, MenuItem.id == OrderItem.menu_item_id).join(Order, OrderItem.order_id == Order.id).filter(
            Order.created_at >= start_date,
            Order.created_at < end_date,
            Order.status == OrderStatus.completed
        ).group_by(Category.name).order_by(desc('total_revenue_usd')).all()
            ...
            'top_selling_items': [
                {
                    ...
                    'total_revenue_usd': round(float(item.total_revenue_usd), 2)
...
def get_sales_by_item():
    ...
    try:
        ...
        item_sales = db.session.query(
            ...
            func.sum(OrderItem.line_total_lbp_rounded_at_order).label('total_sales_lbp'),
        ...
        ).group_by(OrderItem.menu_item_name).order_by(desc('total_sales_usd')).all()
...
def get_sales_by_category():
    ...
    try:
        ...
        category_sales = db.session.query(
            ...
            func.sum(OrderItem.discount_amount_usd).label('total_discounts_usd'),
            func.sum(OrderItem.discount_amount_lbp).label('total_discounts_lbp')
        ).join(Order).join(MenuItem).join(Category).filter(
            Order.created_at >= start_date,
            Order.created_at < end_date,
            Order.status.notin_(['cancelled_by_user', 'cancelled_by_staff'])
        ).group_by(Category.name).order_by(desc('total_sales_usd')).all()
...
def get_discount_usage():
    ...
    try:
        ...
        order_discounts = db.session.query(
        ...
        ).group_by(OrderDiscount.discount_name).all()

        # Query item-level discounts
        item_discounts = db.session.query(
            OrderItemDiscount.discount_name,
            func.count(OrderItemDiscount.id).label('usage_count'),
            func.sum(OrderItemDiscount.discount_amount_usd).label('total_discount_usd'),
            func.sum(OrderItemDiscount.discount_amount_lbp).label('total_discount_lbp')
        ).filter(
            OrderItemDiscount.created_at >= start_date,
            OrderItemDiscount.created_at < end_date
        ).group_by(OrderItemDiscount.discount_name).all()
        ...
        for discount in item_discounts:
            if discount.discount_name in discount_summary:
                discount_summary[discount.discount_name]['item_level_usage'] = discount.usage_count
                discount_summary[discount.discount_name]['total_usage'] += discount.usage_count
                discount_summary[discount.discount_name]['total_discount_usd'] += float(discount.total_discount_usd)
                discount_summary[discount.discount_name]['total_discount_lbp'] += discount.total_discount_lbp
            else:
                discount_summary[discount.discount_name] = {
                    'discount_name': discount.discount_name,
                    'order_level_usage': 0,
        ...
        discounts_data = sorted(list(discount_summary.values()), 
                               key=lambda x: x['total_discount_usd'], reverse=True)
...
def get_hourly_sales():
    ...
    try:
        ...
        if date_str:
            ...
            target_date = datetime.datetime.utcnow().replace(hour=0, minute=0, second=0, microsecond=0)
        ...
        for hour in range(24):
            ...
            hour_sales_usd = sum(float(order.final_total_usd) for order in hour_orders if order.status != 'pending_payment')
            hour_sales_lbp = sum(order.final_total_lbp_rounded for order in hour_orders if order.status != 'pending_payment')
            order_count = len([order for order in hour_orders if order.status != 'pending_payment'])
