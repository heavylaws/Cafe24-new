...
def create_order():
    ...
    try:
        ...
        exchange_rate = (
            get_current_exchange_rate()
        )  # From helpers, ultimately from config/DB
        ...
        try:
            for item_data in data["items"]:
                ...
                chosen_option_id_val = item_data.get(
                    "chosen_option_choice_id"
                )  # Can be None
                ...
                if chosen_option_id_val is not None:
                    ...
                    unit_price_usd = (
                        option_choice.price_usd
                    )  # Option price is the absolute price for this variant
                    chosen_option_name = option_choice.choice_name

                unit_price_lbp_rounded = calculate_lbp_price(
                ...
                line_total_lbp_rounded = (
                    unit_price_lbp_rounded * quantity
                )  # Sum of rounded unit prices
            ...
            final_total_lbp_rounded = calculate_lbp_price(
                order_total_usd, exchange_rate
            )  # Round the final USD sum
    ...
    try:
        ...
        for order in orders:
            ...
            for (
                ...
                order.order_items
            ):  # Assumes 'items' is the relationship name in Order model
            ...
            result.append(
                {
                    ...
                    "exchange_rate_at_order_time": float(
                        order.exchange_rate_at_order_time
                    ),
...
def update_order_status(order_id):
    ...
    current_app.logger.info(
        f"Attempting to update order {order_id} from '{order.status.value}' to '{new_status}' by user with role '{current_user.role.value}'."
    )
    ...
    allowed_transitions = {
        ...
        "paid_waiting_preparation": {"preparing": ["barista", "manager"]},
        "preparing": {"ready_for_pickup": ["barista", "manager"]},
        "ready_for_pickup": {
            "completed": [
                "cashier",
                "courier",
                "manager",
            ]  # Any of these can complete it
        },
        # Add cancellation logic later if needed for v0.1, e.g. manager can cancel
    }

    if (
        order.status.value not in allowed_transitions
        or new_status not in allowed_transitions[order.status.value]
        or current_user.role.value
        not in allowed_transitions[order.status.value][new_status]
    ):
        current_app.logger.error(
            f"403 Forbidden: Cannot change status from '{order.status.value}' to '{new_status}' by user with role '{current_user.role.value}'."
        )
        return (
            jsonify(
                {
                    "message": f"Cannot change status from {order.status.value} to {new_status} with role {current_user.role.value}"
                }
            ),
            403,
        )
    ...
    try:
        ...
        if (
            new_status == "paid_waiting_preparation"
            and current_user.role.value == "cashier"
        ):
...
@jwt_required()
@roles_required(
    "cashier", "manager", "courier"
)  # Courier might want to check status too
