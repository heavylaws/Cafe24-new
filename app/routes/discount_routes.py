...
from app.models import Discount, Order, OrderDiscount, OrderItem, OrderItemDiscount, db
...
def get_discounts():
    ...
    try:
        discounts = (
            Discount.query.filter_by(is_active=True).order_by(Discount.name).all()
        )
        result = []
    ...
    try:
        ...
        if "discount_value" in data:
            ...
            if discount.discount_type == "percentage" and discount_value > 100:
                return (
                    jsonify({"message": "percentage discount cannot exceed 100%"}),
                    400,
                )
...
def apply_order_discount():
    ...
    try:
        ...
        if discount.applies_to != "order":
            return (
                jsonify(
                    {"message": "This discount can only be applied to individual items"}
                ),
                400,
            )
        ...
        discount_amount_lbp = calculate_lbp_price(
            discount_amount_usd, order.exchange_rate_at_order_time
        )
        ...
        order.final_total_lbp_rounded = (
            order.subtotal_lbp_rounded - order.discount_total_lbp_rounded
        )
...
def apply_item_discount():
    ...
    try:
        ...
        if discount.applies_to != "item":
            return (
                jsonify(
                    {"message": "This discount can only be applied to entire orders"}
                ),
                400,
            )
        ...
        existing_discount = OrderItemDiscount.query.filter_by(
            order_item_id=order_item.id, discount_id=discount.id
        ).first()
        if existing_discount:
        ...
        # Calculate discount amount
        if discount.discount_type == "percentage":
            discount_amount_usd = order_item.line_total_usd_at_order * (
                discount.discount_value / 100
            )
        else:  # fixed_amount
            discount_amount_usd = min(
                discount.discount_value, order_item.line_total_usd_at_order
            )

        discount_amount_lbp = calculate_lbp_price(
            discount_amount_usd, order_item.order.exchange_rate_at_order_time
        )
