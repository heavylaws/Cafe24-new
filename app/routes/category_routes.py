...
def update_category(cat_id):
    ...
    name = data.get('name', '').strip()
    if name:
        dup = Category.query.filter_by(parent_id=data.get('parent_id', cat.parent_id), name=name).first()
...
def delete_category(cat_id):
    ...
    if cat.children.count():
        return jsonify({'message': 'Category has subcategories; delete them first'}), 400
    if cat.menu_items.count():
        return jsonify({'message': 'Category has menu items; reassign or delete them first'}), 400
