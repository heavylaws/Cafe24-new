...
def calculate_lbp_price(price_usd, exchange_rate=None, rounding_factor=None):
    ...
    rounded_lbp = (
        lbp_unrounded / Decimal(rounding_factor)
    ).to_integral_value(rounding=ROUND_HALF_UP) * Decimal(rounding_factor)
...
def generate_customer_number():
    ...
    highest_customer = (
        Order.query.filter(Order.customer_number.like(today_pattern))
        .order_by(Order.customer_number.desc())
        .first()
    )
